...
from shapely.geometry import LineString,Point,MultiLineString,MultiPoint,MultiPolygon,LinearRing, Polygon,GeometryCollection
...
class ShapelyPBDeserializer:
    ...
    def deserialize_polygon(self,gpbGeometry) -> Polygon:
        ...
        if(geometries_count == 0):
            ...
            points = [self.coordinate_to_point(coord) for coord in exterior_ls.coordinates]
            return Polygon(points)
        else:
            exterior_ls = gpbGeometry.geometries[0]
            exterior_ls_coordinates = [self.coordinate_to_point(coord) for coord in exterior_ls.coordinates]

            interior_linear_rings_gpb = gpbGeometry.geometries[1].geometries
            
            interior_linear_rings = []
            for ring in interior_linear_rings_gpb:
                interior_linear_rings.append(self.gpb_coords_to_xyz_tuples(ring.coordinates))
            return Polygon(exterior_ls_coordinates,interior_linear_rings)
    
    def deserialize_multi_point(self,gpbGeometry) -> MultiPoint:
        points = [self.deserialize_point(geometry) for geometry in gpbGeometry.geometries]
    ...
    def deserialize_multi_polygon(self,gpbGeometry) -> MultiPolygon:
        polygons = [self.deserialize_polygon for geometry in gpbGeometry.geometries]
        return MultiPolygon(polygons)

    def deserialize_geometry_collection(self,gpbGeometry) -> GeometryCollection:
        geometries = [self.deserialize_geometry(geometry) for geometry in gpbGeometry.geometries]
    ...
    def deserialize_triangle(self,geometry: gpb.Geometry):
        ...
        return Polygon(shell=[self.gpb_coords_to_xyz_tuples(coord) for coord in coordinates])
    ...
    def gpb_coords_to_xyz_tuples(self,gpb_coords):
        return [(coord.x,coord.y,coord.z) for coord in gpb_coords]
